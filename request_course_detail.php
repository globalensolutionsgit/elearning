...
<?php
                $query = mysql_query("select * from class_schedules
                                        JOIN student_teacher_allocation on student_teacher_allocation.schedule_id = class_schedules.class_schedules_id 
                                        ...
                                        JOIN users ON users.user_id = class_schedules.teacher_id where class_schedules.class_schedules_id = '$get_id'")or die(mysql_error());
                $row = mysql_fetch_array($query);
                $id = $row['student_teacher_allocation_id'];
                $count_row_class=mysql_num_rows($query);
               if($count_row_class!=0) {
                ?>
